...
export class BookingFormComponent implements OnInit, OnChanges {
  ...
  onSubmit(ds: HTMLInputElement, df: HTMLInputElement) {
    ...
    if (localStorage.getItem("currentUser")) {
      this.book = {
        ...
        UserId: this.user.Id,
        Sum: Math.ceil(
          this.service.getCarPrice(
            this.service.car,
            this.book.DateStart,
            this.book.DateFinish
          )
        ),
        DateStart: this.getExtraTime(
          this.book.DateStart,
          this.bookingForm.value.Time
        ),
        DateFinish: this.getExtraTime(
          this.book.DateFinish,
          this.bookingForm.value.TimeOff
        ),
        Price: 0,
  ...
  setMinTime(e) {
    if (
      this.book.DateStart.toDateString() == this.book.DateFinish.toDateString()
    ) {
      this.minTime = this.getTimes(0)
        .map(x => x.getHours())
        .indexOf(new Date(e.target.value).getHours());
    } else {
      this.minTime = 0;
    }
  }
  ...
  ngOnInit() {
    ...
    this.book.DateFinish = this.service.DateFinish
      ? this.service.DateFinish
      : null;
    this.book.DateStart = this.service.DateStart
      ? this.service.DateStart
      : null;
    this.getTimes();
    if (localStorage.getItem("currentUser")) {
    ...
    this.bookingForm = this.formBuilder.group({
      ...
      Place: [
        this.service.StartPoint ? this.service.StartPoint : "",
        Validators.required
      ],
      PlaceOff: [
      ...
      Time: [
        this.service.DateStart
          ? new Date(1, 1, 1, this.service.DateStart.getHours())
          : this.times[0]
      ],
      TimeOff: [
        this.service.DateFinish
          ? new Date(1, 1, 1, this.service.DateFinish.getHours())
          : this.times[0]
      ],
      Coment: [""]
    });
    ...
    this.router.events.subscribe(evt => {
      ...
      if (evt.url.indexOf("booking") > -1) {
        if (
          this.service.car.Id != Number(evt.url.split("/")[2].split("?")[0])
        ) {
    ...
    this.service.GetCar(id).subscribe(data => {
      if (data) {
        ...
        this.service.car.Books.forEach(b => {
          ...
          this.invalidIntarvals.push({
            DateStart: b.DateStart,
            DateFinish: b.DateFinish
          });
        });
        this.route.queryParamMap.subscribe(data =>
          this.chooseNewSale(Number(data.get("saleId")))
        );
      }
  ...
  checkSale() {
    if (this.sale && this.sale.Id != 0 && this.sale.DaysNumber != 0) {
      if (this.book.DateStart && this.book.DateFinish) {
        if (
          (new Date(this.book.DateFinish).getTime() -
            new Date(this.book.DateStart).getTime()) /
            86400000 <
          this.sale.DaysNumber
        ) {
  ...
  checkDates() {
    if (
      this.book.DateStart &&
      this.book.DateFinish &&
      this.book.DateStart.getTime() > this.book.DateFinish.getTime()
    ) {
